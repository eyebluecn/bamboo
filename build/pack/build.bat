...
@REM ##########################################################################
@REM
@REM  Bamboo build script for Windows
...
ECHO GOOS: %GOOS%
@REM  service dir eg. D:\Group\eyeblue\bamboo\build\pack
SET PACK_DIR=%CD%
ECHO PACK_DIR: %PACK_DIR%
@REM  build dir eg. D:\Group\eyeblue\bamboo\build
FOR %%F IN (%CD%) DO SET BUILD_DIR_SLASH=%%~dpF
SET BUILD_DIR=%BUILD_DIR_SLASH:~0,-1%
ECHO BUILD_DIR: %BUILD_DIR%
@REM project dir eg. D:\Group\eyeblue\bamboo
FOR %%F IN (%BUILD_DIR%) DO SET PROJECT_DIR_SLASH=%%~dpF
SET PROJECT_DIR=%PROJECT_DIR_SLASH:~0,-1%
ECHO PROJECT_DIR: %PROJECT_DIR%

@REM  final zip file name. eg. bamboo-x.x.x.windows-amd64.zip
SET FILE_NAME=%VERSION_NAME%.%GOOS%-%GOARCH%.zip
ECHO FILE_NAME: %FILE_NAME%
@REM  zip dist dir eg. D:\Group\eyeblue\bamboo\tmp\dist
SET DIST_DIR=%PROJECT_DIR%\tmp\dist
ECHO DIST_DIR: %DIST_DIR%
@REM  component dir eg. D:\Group\eyeblue\bamboo\tmp\dist\bamboo-x.x.x
SET COMPONENT_DIR=%DIST_DIR%\%VERSION_NAME%
ECHO COMPONENT_DIR: %COMPONENT_DIR%
@REM  final dist path eg. D:\Group\eyeblue\bamboo\tmp\dist\bamboo-x.x.x.windows-amd64.zip
...
ECHO copy .\bamboo.exe %COMPONENT_DIR%
copy .\bamboo.exe %COMPONENT_DIR%
